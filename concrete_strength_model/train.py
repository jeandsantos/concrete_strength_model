...
import numpy as np
from config.core import PATH_MODELS, config
...
from utils.scoring import calculate_scores, scorers
...
def main():
    ...
    X_train, X_test, y_train, y_test = dm.load_dataset().split_data(
    ...
    if config.config_app.mlflow_bool:
        ...
        os.environ.setdefault(
            key="mlflow_tracking_uri", value=config.config_app.mlflow_tracking_uri
        )
        ...
        experiment_id = mlflow.create_experiment(
            f'{config.config_app.mlflow_experiment_name}{ts.strftime(r"_%Y%m%d_%H%M%S")}',
            artifact_location="./artifacts",
            tags={"project": config.config_app.project_name},
        )

        print(
            f"Created Experiment '{config.config_app.mlflow_experiment_name}' | ID: {experiment_id}"
        )
    ...
    for model_name in model_objects.keys():
        ...
        if config.config_app.mlflow_bool:
            mlflow.start_run(
                ...
                run_name=model_name + ts.strftime(r"_%Y%m%d_%H%M%S"),
        ...
        rs = RandomizedSearchCV(
            ...
            scoring={
                "mse": make_scorer(
                    score_func=mean_squared_error, squared=True, greater_is_better=False
                ),
                "rmse": make_scorer(
                ...
                "mae": make_scorer(
                    score_func=mean_absolute_error, greater_is_better=False
                ),
                "mape": make_scorer(
                    score_func=mean_absolute_percentage_error, greater_is_better=False
                ),
        ...
        for y_obs, x, suffix in [
            (y_train, X_train, "_train"),
            (y_test, X_test, "_test"),
        ]:
            ...
            scores = calculate_scores(
                ...
                rs.predict(x),
                scorers,
                verbose=config.config_app.bool_verbose,
                suffix=suffix,
            )
        ...
        if all_scores["rmse_test"] < best_model_metric:
            ...
            # Export model
            save_pipeline(
                pipeline_to_persist=rs, file_path=PATH_MODELS / "best_model.pkl"
            )
